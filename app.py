...
版本 v1.24 — 25 模块完整链路: 从数据浏览到智能工作流 + AI Copilot
"""
...
with tab_about:
    ...
    with col1:
        ...
        **免费卫星数据 + AI 主导分析 = 不写代码做科研级遥感分析**

        - 🛰️ **真实卫星数据**: Sentinel-2 (10m) + Landsat 4-9 (30m, 1982-至今)
        - 🤖 **AI 对话助手**: 首页即问 — 遥感知识/平台引导/结果解读 (RAG 知识库)
        - ⚡ **AI 自动执行**: 自然语言描述 → 自动填充 → 自动执行全链路分析
        - 🧠 **AI 智能解读**: 25 模块中 16+ 模块分析后自动生成专业解读
        - 👁️ **AI 影像理解**: 光谱指纹"看图说话" + 异常检测 + 质量诊断
        - 📝 **AI 全文报告**: 一键生成完整科研报告 (摘要/结论/建议)
        - 🎯 **分类体系**: 监督 (RF/SVM/KNN/MLP) + 非监督 (KMeans) + GLCM 纹理
        - 🗺️ **Geo AI 空间分析**: AI 智能缓冲区 + 叠加分析 (直接分析平台结果)

        ### 🎯 适用场景
        - 干旱区水资源监测 (水体 + 干旱指数 + LST)
        - 绿洲植被覆盖变化 (NDVI 时序 + Sen+MK + BFAST 突变)
        - 冰川/积雪消融追踪 (冰冻圈)
        - 土壤盐渍化监测 (NDSI + 5级评估)
        - 蒸散发与水资源平衡 (SEBAL)
        - 沙漠化监测与生态安全 (PSR + 空间叠加)
    ...
    with col2:
        ...
        | 分析 | NumPy + SciPy + Rasterio + statsmodels |
        | AI | **DeepSeek** (对话/解读/报告) + PyTorch (分割) |
        | 检索 | RAG 遥感知识库 (35 条指数/术语) |
        | 分类 | scikit-learn + KMeans + GLCM |
        ...
        # 预设研究区清单 (放 col2 内)
        st.markdown("### 🔬 预设研究区")
        area_list = "、".join(f"📍 {n}" for n in STUDY_AREAS)
        st.markdown(area_list)
