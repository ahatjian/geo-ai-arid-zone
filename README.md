...
> v1.24 | Phase 1-6 全部完成 | 25 分析模块 + DeepSeek AI 智能查询与解读 + AI 研究规划 Copilot + 离线演示模式 + 影像预处理 + PDF 报告 + 自定义 AOI + 矢量导出 + 数据下载中心
...
- 🌤️ **辐射定标与大气校正** — DOS 暗像元法大气校正（Chavez 1988/1996）+ 辐射定标（DN→TOA）+ 相对辐射归一化
- ⚡ **BFAST 断点检测** — STL 去季节 + PELT 断点检测 + Chow 显著性检验（植被突变/退化/恢复事件识别）
- 🧵 **GLCM 纹理分析** — 灰度共生矩阵 6 种纹理特征（对比度/同质性/能量等），监督分类辅助特征（实测提升 OA 20 个百分点）
- 🗺️ **空间邻域分析（Geo AI）** — AI 智能缓冲区（分类图选目标→缓冲梯度→DeepSeek 解读）+ 邻域统计 + 叠加交叉表，直接分析平台结果而非外部 GIS 数据
- 🤖 **AI 智能助手** — AI 对话（遥感知识/平台引导/结果解读）+ AI 一键分析（自动算指数→判地物→综合评估）+ AI 异常检测（局部异常扫描→成因解读）+ 👁️ AI 影像理解（光谱指纹看图说话）+ 🏥 AI 质量诊断（云/噪声/异常自动评估）
...
- **测试**: 554 用例通过 (1 项按条件跳过) | **覆盖率**: 63% | **部署**: Streamlit Cloud ✅
